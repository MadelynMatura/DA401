...
```{r}
plot2 <- ggplot(Total20,aes(GHE_percent, Adult_mort)) + geom_point(aes(col=Income_Group)) + labs(y= "Adult Mortality Rate per 1000 people", x= "Government Healthcare Expenditures as a Percent of GDP") + geom_smooth(method= "lm")
plot2
```


```{r}
lm2 <- lm(Adult_mort ~ GHE_percent, data=Total20) 
summary(lm2)
```
...
```{r}
national_df <- get_data("UHC_INDEX_REPORTED")
...
```{r}
plot6 <- ggplot(Total21,aes( Spending_per_capita_log, Disease_Burden_Index_Scaled)) + geom_point() + labs(y= "Disease Burden Index", x= "Log Government Healthcare Spending per Capita (USD)") + geom_smooth(method = "lm")
plot6
```


```{r}
plot3 <- ggplot(Total20,aes(Income_Group, Spending_per_capita)) + geom_bar(stat = "summary") + labs(y= "Disease Burden Index", x= "Log Government Healthcare Spending per Capita (USD)") 
```


```{r}
lm4 <- lm(Disease_Burden_Index_Scaled ~ GHE_percent + Income_Group + region, data=Total20) 
summary(lm4)
```
