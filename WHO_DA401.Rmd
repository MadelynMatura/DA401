...
```{r}
plot5 <-ggplot(data=TotalB, aes(x= reorder(Income_Group, n), y= n)) + 
  geom_bar(stat="summary", fill = "red") + labs(x= "Income Groups", y = "Count")
plot5
```
...
```{r}
#test <- chisq.test(table(Total20$Income_Group, Total20$region))
#test

test2 <- fisher.test(Total20$Income_Group, Total20$region)
test2
...
```{r}
anova_two_way <- aov(Spending_per_capita~Income_Group, data = Total20)
...
```{r}
anova_two_way <- aov(Spending_per_capita~region, data = Total20)
summary(anova_two_way)
...
p10 <- ggplot(Total31, aes(x = Universal_Legislation, y = Spending_per_capita)) +
        geom_boxplot(fill= "light blue") + labs (x= "Passed Universal Healthcare Legislation",  y= "Healthcare Spending per Capita (USD)")
...
p10 <- ggplot(Total_Africa, aes(x = Universal_Legislation, y = Disease_Burden_Index_Scaled)) +
        geom_boxplot(fill= "red") + labs (x= "Passed Universal Healthcare Legislation in African Countries",  y= "Disease Burden Index in African Countries")
...
```{r}
p10 <- ggplot(Total_Europe, aes(x = Universal_Legislation, y = Disease_Burden_Index_Scaled)) +
        geom_boxplot(fill= "blue") + labs (x= "Passed Universal Healthcare Legislation in European Countries",  y= "Disease Burden Index in European Countries")
...
#Americas Dataset
```{r}
Total_Americas <- Total31 %>%
  filter(region== "Americas")

mean(Total_Americas$Disease_Burden_Index_Scaled)
...
Total_HighIncome <- Total31 %>%
  filter(Income_Group == "High-income" | Income_Group=="Upper-middle-income")
...
```{r}
p10 <- ggplot(Total_HighIncome, aes(x = Universal_Legislation, y = Disease_Burden_Index_Scaled)) +
        geom_boxplot(fill= "light blue") + labs (x= "Passed Universal Healthcare Legislation",  y= "Disease Burden Index")
...
```{r}
p10 <- ggplot(Total31, aes(x = Universal_Legislation, y = Adult_mort)) +
        geom_boxplot(fill= "light blue") + labs (x= "Passed Universal Healthcare Legislation",  y= "Adult Mortality Rate (per 1000 people in 2016)")
...
```{r}
t.test(Adult_mort ~ Universal_Legislation, data = Total31)
```
